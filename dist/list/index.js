import React, { forwardRef, useRef, useState, useMemo, useCallback, useEffect, useImperativeHandle, createElement } from 'react';
...
function usePositionTracker(containerRef) {
  const posRef = useRef(/* @__PURE__ */ new Map());
...
var AnimateList = forwardRef(function AnimateList2({
  ...
  const ghostAnimatedRef = useRef(/* @__PURE__ */ new Set());
  const ghostTimersRef = useRef(/* @__PURE__ */ new Map());
  ...
  const animateCollection = useCallback((source = activeTrigger) => {
    var _a2, _b2;
  ...
  const runSingleItem = useCallback((key) => {
    var _a2, _b2;
    const root = rootRef.current;
  ...
  if (marquee) {
    ...
    return createElement(
      ...
      createElement(
        ...
        marqueeChildren.map(
          (child, index) => createElement("div", { key: `mq-${index}`, style: { display: "inline-flex", alignItems: "center" } }, child)
        )
      )
    );
  }
